const baseURL = "http://192.168.43.80/api";
//USER ROUTE CONSTANTS
const loginURL = baseURL + '/login';
const registerURL = baseURL + '/register';
const userURL = baseURL + '/user';
const baseAssetURL = "http://192.168.43.80/";
const defaulImageURL = baseAssetURL + 'image.jpg';

//CHURCH ROUTE APP CONSTANT
const churchURL = baseURL + '/churches';
const preachinghURL = baseURL + '/preachings/church/';
const eventsURL = baseURL + '/events';
const likeEventURL = baseURL + '/event/';

const serverError = "Erreur du serveur";
const unauthorized = "Unthorized";
const someThingWentWorng = "Some thing wrong tray again !";
